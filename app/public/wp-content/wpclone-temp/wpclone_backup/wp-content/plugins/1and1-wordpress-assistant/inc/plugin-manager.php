...
class One_And_One_Plugin_Manager {
	...
	/**
	 * One_And_One_Plugin_Manager constructor.
	 *
	...
	public function setup_plugins( $plugin_slugs ) {
		...
		new One_And_One_Plugin_Adapter();

		// If not cached plugins available, then update cache file
		$plugin_meta_filename = One_And_One_Wizard::get_plugin_dir_path() . 'cache/plugin-'. $this->site_type . '-meta.txt';
		...
		foreach ( One_And_One_Installer::get_plugin_installation_paths( $plugin_slugs ) as $plugin_path => $plugin_slug ) {
	...
	public function setup_theme( $theme_slug ) {
		...
		if ( ! empty( $theme_slug ) ) {
			...
			if ( ! array_key_exists( $theme_slug, $installed_themes ) ) {
				...
				include_once( One_And_One_Wizard::get_inc_dir_path() . 'installer.php' );
				One_And_One_Installer::install_theme( $themes_meta[ $theme_slug ] );
			}
		...
		if ( isset( $plugin_info->response ) ) {
			foreach ( $plugin_info->response as $plugin_path => $plugin ) {
				...
				if ( in_array( $plugin->slug, $plugin_slugs ) ) {
					One_And_One_Installer::update_plugin( $plugin_path );
				}
			}
		}
	}
	...
	public function install_missing_plugins( $plugin_slugs ) {
		...
		foreach ( $plugin_slugs as $plugin_slug ) {
			...
			if ( ! in_array( $plugin_slug, $plugins_installed ) ) {
				One_And_One_Installer::install_plugin( $this->plugin_meta[ $plugin_slug ] );
			}
		}
	}
	...
	public function get_theme_meta() {
		$themes              = array();
		$theme_meta_filename = One_And_One_Wizard::get_plugin_dir_path() . 'cache/theme-'. $this->site_type . '-meta.txt';
